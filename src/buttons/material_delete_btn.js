...
class DeleteBtn extends React.Component {
  ...
  setIcon(){
    let size = (this.props.size) ? this.props.size : "md-24";

    return `material-icons ${size}`;
  }

  render(){
    return (
      <OverlayTrigger placement="left" overlay={<Tooltip>{this.props.tooltip}</Tooltip>}>
      <button className="btn btn-danger btn-sm" type="button" onClick={this._onClick.bind(this)} title={this.props.tooltip}><i className={this.setIcon()}>delete</i></button>
      </OverlayTrigger>
    );
  }
